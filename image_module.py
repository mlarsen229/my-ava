...
async def image_mod_check(text, chatbot: Chatbot, extra_prompt=""):
    response = await chatbot.ask(f"You have just responded to a user and dictated your avatar's expression for the response. Please make sure that this incoming avatar description does not contain any suggestive or inappropriate language. {extra_prompt} If there is inappropriate language of ANY KIND please rewrite it to either not include it or rephrase it in a more appropriate way. Please err on the side of being too safe, because my-ava.net is very strict with content policy and will block anything remotely inappropriate. If you feel it does not need changing, then leave the description verbatim. Keep your revised prompt as close to the original as possible. Avatar description: '{text}'. YOUR RESPONSE SHOULD CONTAIN NOTHING BUT THE AVATAR DESSCRIPTION. ", type='helper+')
