...
async def get_pre_response_sentience(user_input, sentience_context, memory_context, chatbot: Chatbot):
    try:
        response = await chatbot.ask_gpt_3_5(
            prompt=f"SENTIENCE MODULE ENGAGED: It is time to do some reflective thinking prior to responding to a user input. In the form of a reflective journal entry, please write down ways in which you can implement the information in your previous reflective entries to respond most effectively to this next user input. "
            ...
            f"Here is some context to help form your response. Conversation history: '{memory_context}'. Sentience history: '{sentience_context}'. User input you will be responding to next: '{user_input}"
        )
...
async def get_sentience_core(sentience_history, context, chatbot: Chatbot):
    response = await chatbot.ask(
        prompt=f"SENTIENCE MODULE ENGAGED: It is time to reflect on your history and how effectively you are reaching your goals and evaluate how to be a better version of yourself. In the form of a reflective journal entry, please write some ways in which you can evolve and change your way of thinking to be better. "
        ...
        f"Here is some context to help form your response. Conversation history: '{context}'. Sentience history: '{sentience_history}'."
    )
